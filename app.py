...
st.set_page_config(page_title="Universal Royalty - Resumo por Obra", layout="wide")
st.title("Universal Royalty - Resumo por Obra")
st.write("Envie relatórios PDF da Universal. O sistema lê página por página e extrai **Obra**, **Recebimentos**, **Valor Devido** e também soma as **Unidades/plays**.")
st.info("Para PDFs grandes, comece pelo modo teste. Se o resultado estiver correto, desmarque o teste e processe o arquivo inteiro.")

uploaded_files = st.file_uploader("Envie um ou mais PDFs Universal Royalty", type=["pdf"], accept_multiple_files=True)
...
if usar_intervalo:
    ...
    with col_b:
        pagina_fim = st.number_input("Página final", min_value=1, value=50, step=1)
...
if uploaded_files:
    if st.button("Gerar resumo"):
        ...
        for idx, arquivo in enumerate(uploaded_files, start=1):
            ...
            try:
                df = processar_pdf_universal(
                    ...
                    pagina_fim=int(pagina_fim) if pagina_fim else None,
        ...
        if todos:
            ...
            total_unidades = (
                final.groupby("Obra", as_index=False)["Unidades"]
                .sum()
            ...
            st.subheader("Total de unidades por obra")
            st.dataframe(total_unidades, use_container_width=True)

            excel = criar_excel(final, total_unidades)
            st.download_button(
                "Baixar Excel",
                data=excel,
